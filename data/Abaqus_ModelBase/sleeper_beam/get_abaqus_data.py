...
def main(config):
    ...
    for i in range(config.mask_num):
        ...
        model = mdb.Model(name=model_name, objectToCopy=mdb.models['cybel-10mm'])
        part = mdb.models[model_name].parts['PART-1']
        elements = part.sets['up'].elements

        # put material
        damage_mask = np.load(mask_file_path)

        for i in range(len(config.damage_name)):
            damage_index = mask2index(damage_mask==i+1)
            if len(damage_index)==0:
                continue
            damage_elements = elements.sequenceFromLabels(damage_index)
            damage_region = part.Set(elements=damage_elements, name='Set-damage'+str(i))
            #damage_region = get_set(TableMesh, damage_index, config.damage_name[i], type_name='elements')
            part.SectionAssignment(region=damage_region, sectionName=config.damage_section_name[i], offset=0.0, 
                                offsetType=MIDDLE_SURFACE, offsetField='', 
                                thicknessAssignment=FROM_SECTION)
